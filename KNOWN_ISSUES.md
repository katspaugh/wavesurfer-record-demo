...
Open follow-up items for the current architecture (single `recording-sessions` IndexedDB, `react-flow` UI, `Result<T, AppError>` core).
...
| UI test gap | `src/test/*` | Storage, service, hook (`usePipeline`), and URL-state tests are covered, but the flow chart and session library have no rendered tests — `react-flow` drag, handle wiring, edge animation, and library navigation are not exercised. |
